...
main = do
  ...
  train_data_lines  <- liftM groupBy ((==) . (!! 0)) $ fmap lines $ hGetContents =<< openFile (dataLoc ++ "train.data" ) ReadMode
  ...
  test_data_lines  <- liftM groupBy ((==) . (!! 0)) $ fmap lines $ hGetContents =<< openFile (dataLoc ++ "test.data" ) ReadMode
